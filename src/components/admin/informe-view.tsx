...
const fmtDia = new Intl.DateTimeFormat("es-MX", { day: "numeric", month: "short" });

// "ago. de 2026" → "ago 2026" (misma limpieza que "miembro desde" en Cuenta)
function etiquetaMes(mes: string) {
  return fmtMes.format(new Date(`${mes}-15T00:00:00`)).replaceAll(".", "").replace(" de ", " ");
}
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Tile
          ...
          color="f-y"
          pie={totales.altasMes > 0 ? `+${totales.altasMes} este mes` : "sin altas este mes"}
        />
        <Tile titulo="Activas 7 días" valor={totales.activas7} color="f-t" pie="con registros nuevos" />
        <Tile
          titulo="Movimientos"
          valor={totales.movs}
          color="f-b"
          pie={totales.movsMes > 0 ? `+${totales.movsMes} este mes` : "ninguno este mes"}
        />
        <Tile titulo="Metas" valor={totales.metas} color="f-p" pie={`${totales.apartados} apartados`} />
      </div>
          ...
          <div key={a.etiqueta}>
            ...
            <div className="bfill mt-1 overflow-hidden">
              <div
                ...
                style={{ width: `${totales.cuentas > 0 ? Math.round((a.n / totales.cuentas) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <div className="nbs flex flex-col gap-2 px-3.5 py-3">
        {altasPorMes.map((a) => (
          <div key={a.mes} className="flex items-center gap-2.5">
            <span className="w-18 shrink-0 text-[11px] font-extrabold">{etiquetaMes(a.mes)}</span>
            <div className="bfill flex-1 overflow-hidden">
              <div className="f-b h-full" style={{ width: `${Math.round((a.n / maxAltas) * 100)}%` }} />
            </div>
            <span className="w-5 shrink-0 text-right text-[11.5px] font-black">{a.n}</span>
          </div>
        ))}
      </div>
      ...
      {cuentas.map((c) => (
        <div key={c.id} className="nbs px-3.5 py-3">
          ...
          <div className="mt-1 text-[10.5px] font-bold text-muted-foreground">
            ...
            {c.ultimoMov ? `último registro ${etiquetaDia(c.ultimoMov)}` : "sin registros"}
          </div>
        </div>
      ))}
    </>
...
function Tile({ titulo, valor, color, pie }: { titulo: string; valor: number; color: string; pie?: string }) {
  return (
    <div className={cn("stat", color)}>
      <div className="text-[9.5px] font-black uppercase tracking-[0.1em]">{titulo}</div>
      <div className="mt-0.5 text-[21px] font-black tracking-tight">{valor}</div>
