...
export function MisDatos({ email }: { email: string }) {
  ...
  return (
    <>
      <button className="nbs crow text-left" onClick={() => setAbierto("correo")}>
        <span className="text-[17px]">✉️</span>
        <span className="min-w-0 flex-1">
          <b className="block truncate text-[13px] font-extrabold">Correo</b>
          <span className="text-[10.5px] font-bold text-muted-foreground">{email}</span>
        </span>
        {chevronDer}
      </button>
      <button className="nbs crow text-left" onClick={() => setAbierto("contrasena")}>
      ...
      {abierto === "correo" && (
        <CorreoDialogo actual={email} onGuardar={cambiarCorreo} onCerrar={() => setAbierto(null)} />
      )}
      {abierto === "contrasena" && (
        <ContrasenaDialogo
