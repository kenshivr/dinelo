...
export default async function ControlPage() {
  ...
  const [metas, aportes, apartados, categorias, medios, movimientos, transferencias] = await Promise.all([
    ...
    supabase.from("medios").select("id, nombre, emoji, tipo, saldo_inicial").eq("user_id", auth.user.id).order("created_at"),
    // solo lo que el saldo necesita; lo "Sin medio" no se puede atribuir y se queda fuera
    supabase.from("movimientos").select("tipo, monto, medio_id").not("medio_id", "is", null),
    supabase.from("transferencias").select("id, origen_id, destino_id, monto, fecha").order("created_at", { ascending: false }),
  ]);

  const listaMedios = (medios.data ?? []).map(
    (m): Medio => ({ id: m.id, nombre: m.nombre, emoji: m.emoji, tipo: m.tipo ?? "", saldoInicial: m.saldo_inicial }),
  );
  const listaTransferencias = (transferencias.data ?? []).map(
    (t): Transferencia => ({ id: t.id, origenId: t.origen_id, destinoId: t.destino_id, monto: t.monto, fecha: t.fecha }),
  );
  const saldos = saldosPorMedio(
    listaMedios,
    (movimientos.data ?? []).map(
      (v): MovimientoDeSaldo => ({ tipo: v.tipo as MovimientoDeSaldo["tipo"], monto: v.monto, medioId: v.medio_id }),
    ),
    listaTransferencias,
  );
