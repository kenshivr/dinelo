...
import { InformeView, type CuentaInforme, type Informe } from "@/components/admin/informe-view";
...
export default async function AdminPage() {
    ...
    return (
      <>
        ...
        <div className="nbs px-3.5 py-4 text-center">
          ...
          <b className="mt-1 block text-[13.5px] font-black">Falta configurar la llave</b>
          <p className="mt-1 text-[11.5px] font-bold text-muted-foreground">
            Agrega <code className="font-black">SUPABASE_SECRET_KEY</code> en .env.local y en Vercel
            (Dashboard de Supabase → Settings → API Keys → secret key).
          </p>
        </div>
      </>
    );
  ...
  type Resumen = { movs: number; ultimoMov: string | null; metas: number; apartados: number; medios: number };
  const resumen = new Map<string, Resumen>();
  const de = (id: string) => {
    let r = resumen.get(id);
    if (!r) resumen.set(id, (r = { movs: 0, ultimoMov: null, metas: 0, apartados: 0, medios: 0 }));
    return r;
  };
  let movsMes = 0;
  ...
  const correos = new Map((usuarios.data?.users ?? []).map((u) => [u.id, u.email ?? ""]));
  ...
  const altasPorMes = Array.from({ length: 6 }, (_, i) => sumarMes(mesActual, i - 5)).map((mes) => ({
    mes,
  ...
  const informe: Informe = {
    ...
    totales: {
      ...
      activas7: cuentas.filter((c) => c.ultimoMov !== null && c.ultimoMov >= hace7).length,
