...
export default async function ConfPage() {
  ...
  const [categorias, medios, frecuentes, movs, aportes, transfs] = await Promise.all([
    ...
    supabase.from("transferencias").select("origen_id, destino_id").eq("user_id", auth.user.id),
  ]);

  const usos: Usos = { categorias: {}, medios: {}, aportes: {}, transferencias: {} };
  ...
  for (const t of transfs.data ?? []) {
    if (t.origen_id) usos.transferencias[t.origen_id] = (usos.transferencias[t.origen_id] ?? 0) + 1;
    if (t.destino_id) usos.transferencias[t.destino_id] = (usos.transferencias[t.destino_id] ?? 0) + 1;
  }

  return (
    <ConfView
      categorias={(categorias.data ?? []) as Categoria[]}
      medios={(medios.data ?? []).map(
